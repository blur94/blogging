...
import { AlertSVG, CheckSVG, CloseSVG } from './icons/svgs';
	...
	render() {
		...
		return (
			<div
				onClick={closeToast}
				className={twJoin(
					`w-84 fixed bottom-0 right-0 z-[55] mb-10 mr-8 flex cursor-pointer flex-row items-center rounded-lg p-4 text-white shadow-xl`,
			...
			>
				...
				<div className={`flex-1`}>
					...
					{message && (
						<p className={`mt-1 text-sm`} dangerouslySetInnerHTML={{ __html: message }} />
					)}
				</div>
			</div>
		);
